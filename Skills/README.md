...
| [rename-files-date-prefix/](./libraries/rename-files-date-prefix/) | Rename Files with Date Prefix | Renames all files in a SharePoint document library (including subfolders) to prefix them with their created date in `yyMMdd` format (e.g., `251203-Status Report.pdf`). Shows a preview before committing changes, skips already-prefixed files, and handles errors gracefully. Use when asked to prefix filenames with dates or add date prefixes to files. |
